...
interface EditDonationScreenProps {
...
const EditDonationScreen: React.FC<EditDonationScreenProps> = ({ navigation, route }) => {
  const { donation } = route.params;
  const { categories, loadingCategories, updateListing } = useData();
  const { user } = useAuth();
  
  // Form state - initialize with existing donation data
  const [donationTitle, setDonationTitle] = useState(donation.title);
  const [description, setDescription] = useState(donation.description);
  const [selectedCategory, setSelectedCategory] = useState(donation.categoryId);
  const [selectedCondition, setSelectedCondition] = useState('New'); // Default condition
  const [imageUrl, setImageUrl] = useState('');
  const [imageUrls, setImageUrls] = useState<string[]>([]);
  const [selectedImages, setSelectedImages] = useState<string[]>(donation.images || []);
  ...
  // Check if user is a donor or admin
  const isDonor = user?.accountType === 'donor' || user?.roleId === 1;
  const isAdmin = user?.roleId === 3;
  const canEdit = isDonor || isAdmin;

  useEffect(() => {
    // If user is not a donor or admin, show alert and go back
    if (user && !canEdit) {
      Alert.alert(
        'Access Denied',
        'Only donors and admins can edit donations. Please log in with appropriate permissions.',
        [
          { text: 'OK', onPress: () => navigation.goBack() }
        ]
      );
    }
  }, [user, canEdit, navigation]);
  ...
  const handleSubmit = async () => {
    // Check if user is a donor or admin
    if (!canEdit) {
      Alert.alert('Access Denied', 'Only donors and admins can edit donations.');
      return;
    }
    ...
    try {
      // Prepare the listing data for update
      const listingData = {
      ...
      // Update the listing
      await updateListing(donation.id, listingData);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        {/* Access Warning for Non-Donors/Admins */}
        {user && !canEdit && (
          <View style={styles.warningContainer}>
            <Text style={styles.warningText}>
              ⚠️ Only donors and admins can edit donations. Please log in with appropriate permissions.
            </Text>
          </View>
        )}
