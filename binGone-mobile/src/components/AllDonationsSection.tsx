...
interface AllDonationsSectionProps {
  style?: any;
  hiddenSectionHeader?: boolean;
  navigation?: NavigationProp;
}

const AllDonationsSection: React.FC<AllDonationsSectionProps> = ({ style, hiddenSectionHeader = false, navigation }) => {
  ...
  // Fetch all donations when component mounts
  useEffect(() => {
    fetchAllDonations();
  }, []);
  ...
  const fetchAllDonations = async () => {
    ...
    try {
      ...
      setGroupedListings(sortedAvailableData);
      setCollectedListings(sortedCollectedData);
    } catch (error) {
  ...
  const handleCollectAll = async (donationId: string) => {
    try {
      ...
      if (groupedItem && groupedItem.listings.length > 0) {
        ...
        // Show result to user
        if (successCount > 0) {
          console.log(`Successfully collected ${successCount} donations`);
          if (errorCount > 0) {
      ...
      {/* Section Header */}
      {
        !hiddenSectionHeader && (
          ...
          <Text style={styles.sectionTitle}>
            {activeTab === 'listed' ? 'All Donations Listed' : 'Donations Collected'}
          </Text>
          <View style={styles.headerActions}>
            <TouchableOpacity style={styles.refreshButton} onPress={handleRefreshPress}>
              <MaterialIcons name="refresh" size={wp(4)} color={colors.gray.medium} />
            </TouchableOpacity>
            <TouchableOpacity style={styles.calendarButton} onPress={handleCalendarPress}>
              <MaterialIcons name="calendar-today" size={wp(4)} color={colors.gray.medium} />
            </TouchableOpacity>
...
export default AllDonationsSection;
